...
RSpec.describe "OrdersControlleres", type: :request do
  ...
  describe "put order_path with valid data" do
    it "updates an entry and redirects to the show path for the customer" do
      order = FactoryBot.create(:order)
      put order_path(id: order.id), params: {order: {product_count: 80}}
      order.reload
      expect(order.product_count).to eq(80)
  ...
  describe "put order_path with invalid data" do
    it "does not update the order record or redirect" do
      order = FactoryBot.create(:order)
      put order_path(order.id), params: {order: {product_count: 1000}}
      order.reload
      expect(order.product_count).not_to eq(1000)
      expect(response).to render_template(:edit)
    end
  end
